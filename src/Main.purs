...
handleMessage bot msg =
  launchAff_ $ do
      ...
      result <- D.update
                  ...
                  , telegram:
                      { editMessageReplyMarkup: (\p -> editMessageReplyMarkup bot p *> pure unit # liftEffect)
                      , editMessageMedia: (\p -> editMessageMedia bot p *> pure unit # liftEffect)
                      , sendVideo: (\p -> sendVideo bot p # toAffE)
                      , deleteMessage: (\p -> deleteMessage bot p *> pure unit # liftEffect)
